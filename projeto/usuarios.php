...
<head>
    ...
    <title>Chamados - INAS</title>
        ...
        <script type="text/javascript" class="init">
            var setores = [];
            var perfis = [{id: "1", nome: "Administrador"}, {id: "2", nome: "Atendente"}, {id: "3", nome: "Usuário"}];
            <?php
            require_once("./model/ManterSetor.php"); 

            $manterSetor = new ManterSetor();

            $listaSetor = $manterSetor->listar();

            //var_dump($listaPerfil);
            foreach ($listaSetor as $obj) {
                ?>item = {id: "<?= $obj->id ?>", nome: "<?= $obj->sigla ?>"};
                            setores.push(item);
                <?php
            }
            
            ?>
            function carregaSetor(id_atual) {
            ...
            function carregaPerfil(id_atual) {
                var html = '<option value="">Selecione </option>';
            ...
            $(document).ready(function () {
                ...
                $('#usuarios').DataTable({
                    "processing": true,
                    "serverSide": true,
                    "ajax": "get_usuario.php?p=2"
                });
                carregaSetor(0);
                carregaPerfil(0);
            });
            function excluir(matricula, nome) {
                $('#delete').attr('href', 'del_usuario.php?matricula=' + matricula);
                $('#nome_excluir').text(nome);
                $('#confirm').modal({show: true});              
            }
            function alterar(matricula, nome,idsetor,idperfil) {
                $('#matricula').val(matricula);
                $('#nome').val(nome);
                $('#form_usuario').collapse("show");
                $('#altera').val('1');
                $("#cad_senha").attr("placeholder", "Deixe em branco para não alterar");
                $("#cad_confirma_senha").attr("placeholder", "Deixe em branco para não alterar");

                $('#btn_cadastrar').hide();
                carregaSetor(idsetor);
                    ...
                    <!-- Content Row -->
                        ...
                        <div class="card mb-4 border-box" style="max-width:850px">
                            <div class="row ml-0 card-header py-2 bd-titulo" style="width:100%">
                                ...
                                <div class="col mb-0">
                                    <span style="align:left;" class="h6 m-0 font-weight text-white">Usuários</span>
                                </div>
                            ...
                            <div class="card-body">
                                <table id="usuarios" class="table-sm table-striped table-bordered dt-responsive nowrap" style="width:100%">
                                    <thead>
