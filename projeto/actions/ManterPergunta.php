...
class ManterPergunta extends Model {
    ...
    function listar() {
        $sql = "select p.id,p.descricao, (select count(*) from nota as n where n.id_pergunta=p.id) as dep FROM pergunta as p order by p.id";
    ...
    function getPerguntaPorId($id) {
        $sql = "select p.id,p.descricao FROM pergunta as p WHERE id=$id";
        //echo $sql;
        $resultado = $this->db->Execute($sql);
        $dados = new Pergunta();
        while ($registro = $resultado->fetchRow()) {
