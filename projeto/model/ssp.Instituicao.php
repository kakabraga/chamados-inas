...
class SSP {
     ...
     */
    static function data_output($columns, $data, $db, $perfil) {
        $out = array();
        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
                ...
                if (isset($column['formatter'])) {
                    ...
                    } else if (!empty($column['dx'])) {
                        if ($perfil <= 2) {
                            // Total data set length
                            $resDep = self::sql_exec($db, "SELECT COUNT(*)
                                         FROM  aluno WHERE id_instituicao=" . $data[$i]['id']
                            );
                            $recordsTotalDep = $resDep[0][0];

                            $btn = "<div class='editor'><button class='btn btn-primary btn-sm' type='button' onclick='alterar(" . $data[$i]['id'] . ",\"" . utf8_encode($data[$i]['nome']) . "\",\"" . utf8_encode($data[$i]['descricao']) . "\",\"" . utf8_encode($data[$i]['segmento']) . "\",\"" . utf8_encode($data[$i]['tipo']) . "\",\"" . utf8_encode($data[$i]['grupo_moodle']) . "\")'><i class='fas fa-edit'></i></button>&nbsp;&nbsp;";
                            if ($recordsTotalDep == 0) {
                                $btn .= "<button class='btn btn-danger btn-sm' type='button' onclick='excluir(\"" . $data[$i]['id'] . "\",\"" . utf8_encode($data[$i]['nome']) . "\")'><i class='far fa-trash-alt'></i></button>";
                            } else {
     ...
     */
    static function simple($request, $table, $primaryKey, $columns, $perfil = 6) {
        $bindings = array();
...
//        );
        ...
        return array(
            ...
            "data" => self::data_output($columns, $data, $db, $perfil)
        );
    }
     ...
     *  @return array          Server-side processing response array
     */
    static function complex($request, $conn, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null, $perfil = 6) {
        $bindings = array();
        $db = self::db($conn);
